DROP FUNCTION IF EXISTS public.get_dashboard_stats(uuid, uuid, boolean);

CREATE FUNCTION public.get_dashboard_stats(p_user_id uuid, p_unit_id uuid, p_is_admin boolean)
RETURNS jsonb
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  ...
  FROM orders WHERE unit_id = p_unit_id AND status IN ('draft', 'sent') AND deleted_at IS NULL;

  IF p_is_admin THEN
    SELECT COUNT(*) INTO v_pending_redemptions
    FROM reward_redemptions WHERE unit_id = p_unit_id AND status = 'pending';
