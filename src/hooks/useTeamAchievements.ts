    ...
    queryFn: async () => {
      ...
      const result: TeamMember[] = (profiles || [])
        .map(p => {
          ...
          const achievements = calculateAchievements({ totalCompletions, earnedPoints, totalRedemptions });
          const medals = calculateMedals({ completions: userComps.map(c => ({ completed_at: c.completed_at, item_id: c.item_id })) });
        ...
        .sort((a, b) => (b.unlockedAchievements + b.unlockedMedals) - (a.unlockedAchievements + a.unlockedMedals));
