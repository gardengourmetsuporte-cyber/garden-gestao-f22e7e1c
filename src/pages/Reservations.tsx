        ...
        <div className="px-4 py-4 lg:px-8 max-w-[1400px] mx-auto">
          <div className="flex items-center justify-between mb-6">
            <div>
              ...
              <p className="text-sm text-muted-foreground">Gerenciamento de reservas de mesas</p>
            </div>
            <Button onClick={() => setSheetOpen(true)} className="rounded-xl gap-2">
                          ...
                          <div className="flex items-start justify-between">
                            <div className="space-y-1">
                              ...
                              <div className="flex items-center gap-3 text-xs text-muted-foreground">
                                <span className="flex items-center gap-1">
                                  <AppIcon name="Users" size={12} /> {r.party_size} pessoas
                                </span>
                                {r.table_number && (
                                  <span className="flex items-center gap-1">
                                    <AppIcon name="LayoutGrid" size={12} /> Mesa {r.table_number}
                                  </span>
                                )}
                                {r.customer_phone && (
                            ...
                            {r.status === 'confirmed' && (
                              <div className="flex gap-1">
                                <Button variant="ghost" size="sm" className="text-xs" onClick={() => handleStatusChange(r.id, 'seated')}>
                                  Sentar
                                </Button>
                                <Button variant="ghost" size="sm" className="text-xs text-destructive" onClick={() => handleStatusChange(r.id, 'no_show')}>
                                  No-show
                                </Button>
                              </div>
                            )}
                            {r.status === 'seated' && (
                              <Button variant="ghost" size="sm" className="text-xs" onClick={() => handleStatusChange(r.id, 'completed')}>
                                Finalizar
                              </Button>
                            )}
                          </div>
