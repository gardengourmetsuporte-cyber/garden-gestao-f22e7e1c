    ...
    <div className="space-y-1.5">
      {isLoading ? (
        ...
        <div className="flex flex-col items-center justify-center py-16">
          <div className="w-16 h-16 rounded-[20px] flex items-center justify-center mb-4 finance-hero-card checklist-gradient-slow shadow-[0_0_20px_hsl(68_100%_50%/0.2)] border border-primary/20">
...
function CategoryManagerSheet({ open, onOpenChange, categories, onAdd, onUpdate, onDelete }: CategoryManagerSheetProps) {
  ...
  const renderIconGrid = (selectedIcon: string, onSelect: (icon: string) => void) => (
    <div className="space-y-2">
      ...
      />
      <div className="grid grid-cols-7 gap-1 max-h-32 overflow-y-auto rounded-xl border border-border/50 p-1.5 bg-card">
      ...
      <SheetContent side="bottom" className="h-auto max-h-[85vh] rounded-t-3xl overflow-y-auto">
        ...
        <div className="space-y-4 mt-4 pb-4">
          ...
          {categories.length === 0 ? (
            ...
            <div className="space-y-2">
              {categories.map((cat) => (
                <div key={cat.id} className="bg-card rounded-2xl border border-border/50 p-4">
                  {editingId === cat.id ? (
                    <div className="space-y-3">
                      <div className="flex items-center gap-2">
                        <button
