...
export default function TabletHome() {
  ...
  return (
    <div className="fixed inset-0 bg-black overflow-hidden select-none">
      ...
      <div className="absolute left-4 bottom-4 z-10 w-[320px] max-h-[calc(100%-80px)]"
        style={{ bottom: 'max(env(safe-area-inset-bottom, 0px), 16px)', left: 'max(env(safe-area-inset-left, 0px), 16px)' }}
      >
        <div className="bg-white/[0.08] backdrop-blur-2xl rounded-3xl border border-white/[0.12] overflow-hidden shadow-2xl shadow-black/40 flex flex-col max-h-full">
          ...
          <div className="flex flex-col items-center px-6 pt-6 pb-4">
            <div className="w-16 h-16 rounded-2xl overflow-hidden border border-white/15 bg-white/10 backdrop-blur-sm flex items-center justify-center shadow-lg mb-3">
              {logoUrl ? (
                <img src={logoUrl} alt={unit?.name} className="w-full h-full object-cover" />
              ) : (
                <img src={gardenLogo} alt="Garden" className="w-11 h-11 object-contain" />
              )}
            </div>
            <h1 className="text-base font-bold text-white leading-tight text-center">{unit?.name || 'Bem-vindo'}</h1>
          </div>

          {/* Menu items */}
          <div className="flex-1 overflow-y-auto px-3 pb-3" style={{ scrollbarWidth: 'none' }}>
            <div className="rounded-2xl overflow-hidden divide-y divide-white/[0.06]">
              {menuItems.map(item => (
                <button
                  key={item.id}
                  onClick={item.onClick}
                  className="w-full flex items-center gap-3 px-4 py-3.5 text-left hover:bg-white/[0.06] active:bg-white/[0.1] transition-colors group"
                >
                  <div className="w-8 h-8 rounded-xl bg-white/[0.08] flex items-center justify-center shrink-0 group-hover:bg-white/[0.12] transition-colors">
                    <AppIcon name={item.icon} size={18} className="text-white/80" />
                  </div>
                  <div className="flex-1 min-w-0">
                    <p className="text-[13px] font-bold text-white/90">{item.label}</p>
                    <p className="text-[10px] text-white/40 leading-tight">{item.subtitle}</p>
                  </div>
