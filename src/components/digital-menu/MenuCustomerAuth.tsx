...
export function MenuCustomerAuth({ unitName, logoUrl, cuisineType, city, isOpen = true, bonusPoints = 0, fullPage = false, onSkip, onEmailLogin }: Props) {
  ...
  const content = (
    <>
      {/* Logo */}
      <div className="w-24 h-24 rounded-full overflow-hidden bg-white border-[3px] border-border/20 flex items-center justify-center shadow-sm">
        {logoUrl ? (
      ...
      <div className="mt-8 w-full">
        ...
        {onSkip && (
          <button
            onClick={onSkip}
            className="w-full h-14 rounded-xl bg-foreground text-background font-semibold text-sm hover:opacity-90 active:scale-[0.98] transition-all"
          >
            Acessar o cardápio
          </button>
        )}
