            ...
            >
              <div className="finance-hero-inner p-5 pb-4">
                <div className="flex items-center justify-between mb-1">
                  <span className="text-[11px] font-bold tracking-[0.15em] uppercase text-white/50">Saldo da empresa</span>
                  <AppIcon name="ChevronRight" size={18} className="text-white/30" />
                </div>
                <p className={cn(
                  "text-[2rem] font-extrabold tracking-tight leading-tight",
                  stats.monthBalance >= 0 ? "text-white" : "text-red-400"
                )}>
                ...
                {stats.pendingExpenses > 0 && (
                  <div className="flex gap-2 mt-3">
                    <div className="rounded-lg px-3 py-1.5 bg-amber-500/10 border border-amber-500/20">
                      <span className="text-[10px] font-semibold uppercase tracking-wide text-white/50">Pendências</span>
                      <span className="text-sm font-bold text-amber-400 ml-2">{statsLoading ? '...' : formatCurrency(stats.pendingExpenses)}</span>
                    </div>
                  </div>
                )}
              </div>
