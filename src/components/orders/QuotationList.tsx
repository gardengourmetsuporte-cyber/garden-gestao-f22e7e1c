...
type SubTab = 'traditional' | 'global';

export function QuotationList({ suppliers }: Props) {
  const { quotations, isLoading, deleteQuotation } = useQuotations();
  const [sheetOpen, setSheetOpen] = useState(false);
  const [selectedQuotation, setSelectedQuotation] = useState<Quotation | null>(null);
  const [subTab, setSubTab] = useState<SubTab>('traditional');
    ...
    <div className="space-y-4">
      ...
      {subTab === 'traditional' ? (
        ...
        <div className="animate-fade-in">
          <PriceSurveyList suppliers={suppliers} />
        </div>
      )}
    </div>
  );
