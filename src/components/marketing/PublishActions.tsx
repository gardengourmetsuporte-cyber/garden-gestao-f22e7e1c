...
interface PublishActionsProps {
  ...
  onConfirmPublished: (id: string) => void;
}
  ...
  return (
    <Sheet open={open} onOpenChange={onOpenChange}>
      <SheetContent side="bottom" className="space-y-4">
        <SheetHeader>
          <SheetTitle>Publicar Post</SheetTitle>
          <SheetDescription>Escolha o canal e publique com 1 toque</SheetDescription>
        </SheetHeader>
        ...
        <div className="space-y-3">
          {post.channels.includes('instagram') && (
            <Button
              variant="outline"
              className="w-full justify-start gap-3 h-14 border-pink-500/30 hover:bg-pink-500/10"
              onClick={() => handlePublish('instagram')}
            >
              <Instagram className="w-5 h-5 text-pink-400" />
              <div className="text-left">
                <p className="text-sm font-medium">Instagram</p>
                <p className="text-[10px] text-muted-foreground">Abre o app + copia legenda</p>
              </div>
            </Button>
          )}
          ...
          {post.channels.includes('whatsapp_status') && (
            <Button
              ...
              onClick={() => handlePublish('whatsapp_status')}
            >
              <MessageCircle className="w-5 h-5 text-emerald-400" />
              <div className="text-left">
                <p className="text-sm font-medium">WhatsApp Status</p>
                <p className="text-[10px] text-muted-foreground">Compartilha via WhatsApp</p>
              </div>
            </Button>
          )}
        </div>
