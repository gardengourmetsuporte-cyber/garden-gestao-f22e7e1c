...
export function ScheduleManagement() {
  ...
  const handleApprove = async (approved: boolean) => {
    ...
    try {
      ...
      setSelectedSchedule(null); setNotes('');
    } catch { toast.error('Erro ao processar solicitação'); }
    finally { setIsSubmitting(false); }
  };

  const handleDelete = async (id: string) => {
    try { await deleteSchedule(id); toast.success('Folga removida'); setSelectedSchedule(null); }
    catch { toast.error('Erro ao remover folga'); }
  };
  ...
  return (
    <div className="space-y-6">
      ...
      <Sheet open={!!selectedSchedule} onOpenChange={() => setSelectedSchedule(null)}>
        <SheetContent>
          <SheetHeader><SheetTitle>Detalhes da Folga</SheetTitle></SheetHeader>
          {selectedSchedule && (
            <div className="mt-6 space-y-6">
              <div className="flex items-center gap-3">
                <Avatar className="w-12 h-12">
                  <AvatarImage src={selectedSchedule.profile?.avatar_url || ''} />
                  <AvatarFallback><AppIcon name="User" size={24} /></AvatarFallback>
                </Avatar>
                <div>
                  <p className="font-semibold">{selectedSchedule.profile?.full_name || 'Usuário'}</p>
                  <p className="text-sm text-muted-foreground">{selectedSchedule.profile?.job_title || 'Funcionário'}</p>
                </div>
              </div>
              <div className="card-unified p-4 text-center">
                <p className="text-3xl font-bold text-primary">{selectedSchedule.day_off}</p>
                <p className="text-muted-foreground">{monthNames[selectedSchedule.month]} de {selectedSchedule.year}</p>
              </div>
              <div className="flex justify-center">{getStatusBadge(selectedSchedule.status)}</div>
              {selectedSchedule.notes && (
                <div>
                  <p className="text-sm font-medium mb-1">Observação do funcionário:</p>
                  <p className="text-sm text-muted-foreground bg-secondary/50 p-3 rounded-lg">{selectedSchedule.notes}</p>
                </div>
              )}
              {selectedSchedule.status === 'pending' && (
                <div className="space-y-4">
                  <div>
                    ...
                    <Textarea placeholder="Adicionar observação..." value={notes} onChange={(e) => setNotes(e.target.value)} rows={2} />
                  </div>
                  <div className="grid grid-cols-2 gap-3">
                    <Button variant="destructive" onClick={() => handleApprove(false)} disabled={isSubmitting}>
                      {isSubmitting ? <AppIcon name="progress_activity" size={16} className="animate-spin" /> : <AppIcon name="X" size={16} className="mr-2" />}Recusar
                    </Button>
                    <Button onClick={() => handleApprove(true)} disabled={isSubmitting}>
                      {isSubmitting ? <AppIcon name="progress_activity" size={16} className="animate-spin" /> : <AppIcon name="Check" size={16} className="mr-2" />}Aprovar
                    </Button>
                  </div>
                </div>
              )}
              {selectedSchedule.status !== 'pending' && (
                <Button variant="outline" className="w-full" onClick={() => handleDelete(selectedSchedule.id)}>
                  <AppIcon name="X" size={16} className="mr-2" />Remover Folga
                </Button>
              )}
            </div>
          )}
