...
export function EmployeePayments({ employee, onBack }: EmployeePaymentsProps) {
  ...
  const handlePay = async () => {
    if (payDialog.paymentId && selectedAccountId) {
      setIsPaying(true);
      try {
        await markAsPaidWithTransaction({
      ...
      {payments.length === 0 && (
        <div className="text-center py-12 text-muted-foreground">
          ...
          <Button 
            variant="outline" 
            className="mt-4"
            onClick={() => { setEditingPayment(null); setSheetOpen(true); }}
          >
            <Plus className="w-4 h-4 mr-2" />
          ...
          <div className="space-y-4 py-4">
            <div className="space-y-2">
              ...
              <p className="text-xs text-muted-foreground">
                O valor será lançado como despesa nesta conta
              </p>
            </div>
          </div>
          
          <DialogFooter>
            <Button variant="outline" onClick={() => setPayDialog({ open: false, paymentId: null })}>
              Cancelar
            </Button>
